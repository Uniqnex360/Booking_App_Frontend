 ...
 if (loading) {
  ...
  const tabClass = (tab: string) =>
    `px-4 py-2 text-xs font-bold rounded-xl transition flex items-center gap-1.5 ${
      activeTab === tab ? 'bg-[#7B1E3D] text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100'
    }`;
  ...
  {partner?.status === 'APPROVED' ? 'Approved' : partner?.status === 'PENDING_APPROVAL' ? 'Pending Approval' : partner?.status}
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs font-medium text-slate-500 mt-2">
              <span className="flex items-center gap-1"><MapPin className="h-3.5 w-3.5 text-[#7B1E3D]" /> {partner?.city}</span>
              <span className="flex items-center gap-1"><Phone className="h-3.5 w-3.5 text-[#7B1E3D]" /> {partner?.contact_phone}</span>
              <span className="flex items-center gap-1"><Store className="h-3.5 w-3.5 text-[#7B1E3D]" /> {partnerTypeMeta[partner?.partner_type || 'event_organiser'].label}</span>
            </div>
          </div>
          <Button
  ...
  className="bg-[#7B1E3D] hover:bg-[#5C0F2A] text-white font-bold rounded-2xl flex items-center gap-2 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
>
                ...
                <div key={e.id} className="bg-white border border-slate-200 rounded-3xl overflow-hidden flex flex-col justify-between shadow-sm hover:shadow-md transition-all">
                  <div>
                    <div className="aspect-[16/9] bg-slate-100 relative">
                      ...
                      <div className="absolute top-3 right-3">
                        <Badge className={`border ${e.status === 'PUBLISHED' && !isPast ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : e.status === 'PENDING_APPROVAL' ? 'bg-[#7B1E3D]/5 text-[#7B1E3D] border-[#7B1E3D]/30' : 'bg-slate-100 text-slate-600 border-slate-200'}`}>
                          {isPast ? 'COMPLETED' : e.status}
                        </Badge>
                      </div>
                    </div>
                    <div className="p-5">
                      <span className="text-[10px] font-bold text-[#7B1E3D] uppercase tracking-wider">{e.category}</span>
                  ...
                  <div className="p-5 pt-0 border-t border-slate-100 mt-4 flex items-center justify-between">
                    <div className="text-xs text-slate-500 flex items-center gap-1 font-medium">
                      <Ticket className="h-3.5 w-3.5 text-[#7B1E3D]" /> {e.ticket_categories?.length || 1} Tier(s)
                    </div>
          ...
          <DialogContent className="bg-white border-slate-200 text-slate-900 max-w-lg rounded-3xl shadow-2xl">
            ...
            <form onSubmit={handleSaveEdit} className="space-y-4 py-2">
              ...
              <DialogFooter className="gap-2 pt-2">
                ...
                <Button type="submit" disabled={savingEdit} className="bg-[#7B1E3D] hover:bg-[#5C0F2A] text-white font-bold rounded-xl">
